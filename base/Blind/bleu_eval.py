...
def get_reference():
    refer_dataset = []
    with codecs.open('Data/reference.txt','r',encoding='utf-8') as f:
        lines = f.readlines()
        for l in lines:
           ref = []
...
def get_output():
    output_dataset = []
    with codecs.open('Data/output.txt','r',encoding='utf-8') as f:
         flag = 1
         lines = f.readlines()
         for l in lines:
                 l = l.strip()
...
def get_Shixue():
    data = []
    with codecs.open('Data/Shixuehanying.txt',encoding='utf-8') as f:
       lines = f.readlines()
       classes = []
       for l in lines:
...
def get_ci():
   with codecs.open('Data/ci_per_line.txt',encoding='utf-8') as f:
        lines = f.readlines()
        ci = []
        for l in lines:
    ...
    #   print(sen_list)
       ...
       #    print(poten_ref[2])
           if(is_sub_set(sen_list,poten_ref[2])):
                 try:# if poten_ref[1]+1 out of index, then continue
